<?php

require_once 'connection.php';

$id = $_GET['id'];

$q1 = $conn->query("
 SELECT evacuees.*, c.name as calamity, 
 CONCAT(bi.lastname, ', ', bi.firstname) as fullname,
 bi.address as evacAddress,
 bi.age as evacAge,
 bi.gender as evacGender,
 bi.contact_number as evacContact,
 bi.gender as evacGender,
 b.name as evacBrgy,
 CONCAT(bi2.lastname, ', ', bi2.firstname) as head
    FROM evacuees 
   LEFT JOIN calamity c on evacuees.calamity_id = c.id 
   LEFT JOIN barangay_info bi on evacuees.person_id = bi.id 
   LEFT JOIN barangay b on bi.barangay_id = b.id
   LEFT JOIN barangay_info bi2 on evacuees.head_person_id = bi2.id 

  WHERE evacuees.status = 1 and evacuation_center_id = $id and evacuees.is_present = 1
  ORDER BY lastname ASC");

//  $queries = $conn->query($q1);

?>

<script>
    $(function() {
        $(".barangayList").DataTable({
            destroy: true,
            "paging": true,
            "responsive": true,
            "lengthChange": true,
            "autoWidth": false,
            "buttons": ["copy", "csv", "excel", "pdf"]
        }).buttons().container().appendTo('.dataTables_length');
    });
</script>

<table id="barangayList" class="table table-bordered table-hover barangayList">
    <thead>
        <tr>
            <th>#</th>
            <th>Evacuees info</th>
            <th>Barangay</th>

            <th>Head of Family</th>
          

            <th>Calamity</th>
            <th>Days</th>
            <th>Remarks</th>
            <th></th>
        </tr>
    </thead>
    <tbody>
        <?php while ($fetch = $q1->fetch_array()) {
            $evac = $fetch['evacuation_center_id'];
            $calamity_id = $fetch['calamity_id'];

            $q4 = $conn->query("SELECT * FROM evacuation_center WHERE id = $evac");
            $evacuationCenter = $q4->fetch_assoc();

            $q5 = $conn->query("SELECT * FROM calamity WHERE id = $calamity_id");
            $calamityR = $q5->fetch_assoc();

            $now = time();
            $from = strtotime($fetch['date_added']);
            $days = ceil((($now - $from) / 86400) - 1);
        ?>
            <tr class="id_<?php echo $fetch['id'] ?>" id="evacuee_<?php echo $fetch['id'] ?>">
                <td><?php echo $fetch['id'] ?></td>
                <td>
                    <p class="info">Name: <b><?php echo $fetch['fullname'] ?></b></p>
                    <p class="info"><small>Age: <b><?php echo $fetch['evacAge'] ?></b></small></p>
                    <p class="info"><small>Gender: <b><?php echo $fetch['evacGender'] == '1' ? 'Male' : 'Female' ?></b></small></p>
                    <p class="info"><small>Contact: <b><?php echo $fetch['evacContact'] ?></b></small></p>
                </td>
                <td>
                    <?php echo $fetch['evacBrgy'] ?>
                </td>

                <td><?php echo $fetch['head'] ?></td>

                <td><?php echo $calamityR['name'] ?></td>

                <td><?= $days === 0 ? 'Today' : ($days === 1 ?  $days . ' day' : $days . ' days') ?></td>
                <td><?= $fetch['remarks'] ? $fetch['remarks'] : 'None' ?></td>
                <td class="text-center">
                    <div class="form-check">
                        <input id="checkbox" class="form-check-input" type="checkbox" value="" data-id="" id="flexCheckDefault" onchange="removeEvacuee('<?php echo $fetch['id'] ?>')">

                    </div>
                </td>
            </tr>
        <?php } ?>
    </tbody>
</table>

<script>
    const removeEvacuee = (id) => {
        console.log(id)

        $.ajax({
            type: 'post',
            url: 'remove_evacuee.php',
            data: {
                id: id
            },
            success: function(data) {
              
                if (data.success) $(`#evacuee_${id}`).remove();
             
            }

        });


    }
    // $('#checkbox').change(function() {
    //     // let id = $(this).data('id');
    //     if (this.checked) {
    //         var returnVal = confirm("Are you sure?");
    //         $(this).prop("checked", returnVal);
    //         console.log('checked')
    //     }
    //     $('#textbox1').val(this.checked);
    //     console.log('triggered')
    // });
    // $('#checkbox').on('click', function(e) {
    //     e.preventDefault();

    //     // let id = $(this).data('id');

    //     // $.ajax({
    //     //     type: 'post',
    //     //     url: 'delete_mpv_trans.php',
    //     //     data: {
    //     //         id: id
    //     //     },
    //     //     success: function(data) {
    //     //         $('.id_' + id).hide('slow')
    //     //         $('#modal-default').modal('hide');
    //     //     }

    //     // });

    //     console.log(id)
    // })
</script>